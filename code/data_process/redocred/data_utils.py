...
def make_vicuna_train(filelist, save_path):
    train_data = []
    relation_template = "Given the passage : \"{sentences}\", analysis the passage, and list all relations that can be derived from the passage and give the evidence. " \
                        "The relations are :"
    entity_template = "Given relation description : \"{description}\", and the passage : \"{sentences}\". Find all entities in the passage that can be the subject of the " \
                      "relation and give the reason. Subject entities are:"
    fact_template = "Given relation description : \"{description}\", and the passage : \"{sentences}\", the subject of {relation} is : {subject}.\n" \
                    "Now find all objects for {fact}, and give the reasons. Facts are:"
    data = []
    ...
    for sample in data:
        ...
        passage_analysis = sample['passage_analysis']
        block_dict = {
        ...
        for fact in fact_list:
            relation = fact["fact"][1]
            if relation in relation_categories:
                relation_categories[relation].append({"fact": fact["fact"], "explanation": fact["explanation"]})
            else:
                relation_categories[relation] = [{"fact": fact["fact"], "explanation": fact["explanation"]}]
        entity_analysis = sample['entity_analysis']
        for r, f_list in relation_categories.items():
            relation_desc = relation_descript.get(r)
            block_dict = {
                "id": f"identity_{global_id}",
                "conversations": [
                    {
                        "from": "human",
                        "value": entity_template.format(sentences=sentence, description=relation_desc)
                    },
                    {
                        "from": "gpt",
                        "value": str(list(entity_analysis[r].keys()))
                    }
                ]
            }
            train_data.append(block_dict)
            global_id += 1
            for ent in entity_analysis[r].keys():
                block_dict = {
                    "id": f"identity_{global_id}",
                    "conversations": [
                        {
                            "from": "human",
                            "value": fact_template.format(sentences=sentence, description=relation_desc, relation=r, subject=ent, fact=(ent, r))
                        },
                        {
                            "from": "gpt",
                            "value": str([item for item in f_list if item['fact'][0] == ent])
                        }
                    ]
                }
                train_data.append(block_dict)
...
def make_vicuna_train_no_entity_explantion(filelist, save_path):
    train_data = []
    relation_template = "Given the passage : \"{sentences}\", analyze the content, and list all relations that can be derived from the passage along with supporting evidence. " \
                        "The identified relations are as follows: "
    entity_template = "Given relation description : \"{description}\", and the passage : \"{sentences}\". Find all entities in the passage that can act as the subject of the " \
                      "relation. The names of the entities must match exactly with the entity names in the passage. The subject entities are as follows:"
    fact_template = "The passage is : \"{sentences}\", the relation description is : \"{description}\", we have identified the subject as: {subject}.\n" \
                    "Now, the objective is to find all the corresponding objects that fulfill this relationship, and provide an explanation for each identified object. The " \
                    "objects along with their explanations are as follows: "
    data = []
    for file in filelist:
        data = json.load(open(file))
    # data = data['positive'] + data['negative']
            ...
            for ent in entity_analysis[r].keys():
                block_dict = {
                    ...
                    "conversations": [
                        {
                            "from": "human",
                            "value": fact_template.format(sentences=sentence, description=relation_desc, subject=ent)
                        },
                        {
                            "from": "gpt",
                            "value": str([item for item in f_list if item['fact'][0] == ent])
...
def make_vicuna_train_analysis_v0(file, save_path):
    # 这个版本是最细致的分析，所有的结果应该分两步，一个是分析，第二个是list结果。这个可以保证不会eval错误。
    train_data = []
...
def make_vicuna_train_analysis_v1(file, save_path):
    # 这个版本是最细致的分析，所有的结果应该分两步，一个是分析，第二个是list结果。这个可以保证不会eval错误。
    # 和v0比较，模板更新了一下，作为list。
    train_data = []
    relation_template = "Given a passage: \"{sentences}\", conduct a comprehensive analysis to uncover any underlying relations.\n" \
                        "The relations analytical process is as follows: "
    relation_list_template = "List the relations present in the relations_analysis: \"{relation_analysis}\"."
    entity_template = "Given a description: \"{description}\" and a passage: \"{sentences}\", our objective is to extract all entities that can act as the " \
                      "subject of the mentioned relation. The analytical process is outlined below: "
    entity_list_template = "According to the subjects_analysis: \"{subjects_analysis}\", the following entities can be list: "
    fact_template = "Given a description: \"{description}\", the passage: \"{sentences}\", and a specific subject: \"{subject}\", to identify all " \
                    "facts within the text that align with the relation's description and pertain to the mentioned subject. Show the analysis: "
    fact_list_template = "Based on facts_analysis : \"{facts_analysis}\", we have derived the following facts: "
    data = json.load(open(file))
    global_id = 0
    for sample in data:
        sentence = sample['passage']
...
def make_vicuna_train_analysis_v2(file, save_path):
    # 这个版本是最细致的分析，所有的结果应该分两步，一个是分析，第二个是list结果。这个可以保证不会eval错误。
    train_data = []
    relation_template = "Given a passage: \"{sentences}\", conduct a comprehensive analysis to uncover any underlying relations.\n" \
                        "The relations analytical process is as follows: "
    ...
    fact_list_template = "After analyzing the relation description: \"{description}\", the passage: \"{sentences}\", and considering the subject of the relation: \"{subject}\", " \
                         "we have conducted an analysis of the facts as: \"{facts_analysis}\". Based on this analysis, we have derived the following facts: "
    data = json.load(open(file))
    data.extend(json.load(open("../chatgpt/save.json")))
    global_id = 0
    for sample in data:
        try:
            sentence = sample['passage']
...
def make_chatglm2_train_v0(file, save_path):
    train_data = []
    data = json.load(open(file))
    global_id = 0
    relation_template = "Given a passage: \"{sentences}\", conduct a comprehensive analysis to uncover any underlying relations.\n" \
    ...
    for sample in data:
        try:
            ...
            relation_analysis = sample['relation_analysis']
            block_dict = {
                "q": relation_template.format(sentences=sentence),
                "a": str(relation_analysis.replace("\n\n", "\n"))
            }
            train_data.append(block_dict)
            global_id += 1
            block_dict = {
                "q": relation_list_template.format(sentences=sentence, relation_analysis=relation_analysis),
                "a": str("\n".join(list(set(sample['relations']))))
            }
            train_data.append(block_dict)
            global_id += 1
            ...
            for relation, entity_analysis in sample['entity_analysis'].items():
                block_dict = {
                    "q": entity_template.format(sentences=sentence, description=relation_descript.get(relation)),
                    "a": str(entity_analysis.replace("\n\n", "\n"))
                }
                train_data.append(block_dict)
                global_id += 1
                ...
                entity_list = list(set([fact['fact'][0] for fact in sample['fact_list'] if fact['fact'][1] == relation]))
                block_dict = {
                    "q": entity_list_template.format(sentences=sentence, description=relation_descript.get(relation), subjects_analysis=entity_analysis),
                    "a": str("\n".join(entity_list))
                }
                train_data.append(block_dict)
                global_id += 1
                ...
                for subject in entity_list:
                    block_dict = {
                        "q": fact_template.format(sentences=sentence, description=relation_descript.get(relation), subject=subject),
                        "a": str(sample['fact_analysis'][relation][subject].replace("\n\n", "\n"))
                    }
                    train_data.append(block_dict)
                    global_id += 1
                    ...
                    fact_list = [fact['fact'] for fact in sample['fact_list'] if fact['fact'][1] == relation and fact['fact'][0] == subject]
                    block_dict = {
                        "q": fact_list_template.format(sentences=sentence, description=relation_descript.get(relation),
                                                       facts_analysis=sample['fact_analysis'][relation][subject], subject=subject),
                        "a": str("\n".join([str(fact) for fact in fact_list]))
                    }
                    train_data.append(block_dict)
...
def make_chatglm2_train_v2(file, save_path):
    train_data = []
    data = json.load(open(file))
    # data.extend(json.load(open("../chatgpt/save.json")))
    global_id = 0
