...
def process_evaluation_data(evaluation_data, node, cuda_id):
    chunk_size = len(evaluation_data) // 8
    if node == 0:
        start, end = 0, 8
    elif node == 1:
        start, end = 0, 8
        ...
        # start, end = 8, 16
        # start, end = 16, 24
    data = evaluation_data[start * chunk_size:end * chunk_size] if node != 2 else evaluation_data[start * chunk_size:]
...
def get_all_relation_count(redocred_dir, test_relation):
    test_data = json.load(open(f"/zhangpai22/xll/auto_kg/nell162/data/versions/v2/test/{test_relation}.json"))
    ...
    relation_list = json.load(open("./data/auto_kg/relation_list.json"))
...
def get_one_relation_count(redocred_dir, test_relation="educated at"):
    ...
    # print("total", true_relation_count)
    # print("right", right)
    # print( wrong)
    # print("miss", miss)
    json.dump(relation_counts, open(f"{redocred_dir}/get_relation_count.json", "w"), indent=4)
...
def get_relation_recall(redocred_dir):
    ...
    json.dump(sorted_recall, open(f"./test/relation_recall.json", "w"), indent=4)
...
if __name__ == '__main__':
    ...
    model_path = f"/workspace/xll/autokg/ckpt/{mode}/v0/relations/checkpoint-3000"
